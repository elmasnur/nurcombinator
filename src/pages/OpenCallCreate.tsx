  ...
  return (
    <div className="container mx-auto max-w-lg px-4 py-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <Select value={callType} onValueChange={v => setCallType(v as CallType)}>
              ...
              <SelectContent>{Object.entries(CALL_TYPE_LABELS).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div>
            <Label>Konum</Label>
            <Select value={locationMode} onValueChange={v => setLocationMode(v as LocationMode)}>
              <SelectTrigger className="mt-1 bg-secondary border-border"><SelectValue /></SelectTrigger>
              <SelectContent>{Object.entries(LOCATION_MODE_LABELS).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}</SelectContent>
            </Select>
          </div>
        </div>
        <div>
        ...
        <div>
          <Label>Taahhüt</Label>
          <Input value={commitment} onChange={e => setCommitment(e.target.value)} placeholder="Hafta 5 saat" className="mt-1 bg-secondary border-border" />
        </div>
        <div>
        ...
        <div>
          ...
          <Textarea value={description} onChange={e => setDescription(e.target.value)} maxLength={2000} className="mt-1 bg-secondary border-border" rows={4} placeholder="Aradığınız profili ve projenin amacını açıklayın..." />
        </div>
        <div>
          <Label>Etiketler (virgülle ayırın)</Label>
          <Input value={tags} onChange={e => setTags(e.target.value)} placeholder="tasarım, yazılım" className="mt-1 bg-secondary border-border" />
        </div>
